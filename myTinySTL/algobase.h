...
#undef min
	...
	myTinySTL::pair<InputIter, OutputIter>
		unchecked_copy_n(InputIter first, Size n, OutputIter result, myTinySTL::input_iterator_tag)
	{
		...
		return myTinySTL::pair<InputIter, OutputIter>(first, result);
	}

	template <class RandomIter, class Size, class OutputIter>
	myTinySTL::pair<RandomIter, OutputIter>
		unchecked_copy_n(RandomIter first, Size n, OutputIter result,
			myTinySTL::random_access_iterator_tag)
	{
		auto last = first + n;
		return myTinySTL::pair<RandomIter, OutputIter>(last, myTinySTL::copy(first, last, result));
	}
	...
	RandomIter2
		unchecked_move_backward_cat(RandomIter1 first, RandomIter1 last,
			RandomIter2 result, myTinySTL::random_access_iterator_tag)
	{
		for (auto n = last - first; n > 0; --n)
			*--result = myTinySTL::move(*--last);
		return result;
		...
		const unsigned char* last2)
	{
		...
		// 先比较相同长度的部分
		const auto result = std::memcmp(first1, first2, myTinySTL::min(len1, len2));
		// 若相等，长度较长的比较大
	...
	/*****************************************************************************************/
	template <class InputIter1, class InputIter2>
	myTinySTL::pair<InputIter1, InputIter2>
		mismatch(InputIter1 first1, InputIter1 last1, InputIter2 first2)
	{
		...
		return myTinySTL::pair<InputIter1, InputIter2>(first1, first2);
	}
