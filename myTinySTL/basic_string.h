...
// 模板类 basic_string
// 参数一代表字符类型，参数二代表萃取字符类型的方式，缺省使用 myTinySTL::char_traits
	template <class CharType, class CharTraits = myTinySTL::char_traits<CharType>>
	class basic_string
	{
	public:
		...
		typedef myTinySTL::allocator<CharType>               allocator_type;
		typedef myTinySTL::allocator<CharType>               data_allocator;
	...
	public:
		...
		{
			count = myTinySTL::min(count, size_ - index);
	...
	private:
		...
		template <class Iter>
		void          copy_init(Iter first, Iter last, myTinySTL::input_iterator_tag);
		template <class Iter>
		void          copy_init(Iter first, Iter last, myTinySTL::forward_iterator_tag);
		...
		insert(const_iterator pos, Iter first, Iter last)
	{
		...
		if (pos == end())
		{
			myTinySTL::uninitialized_copy(first, last, end());
			size_ += len;
			return r;
		}
		char_traits::move(r + len, r, len);
		myTinySTL::uninitialized_copy(first, last, r);
		size_ += len;
		return r;
	}
	...
	{
		auto n1 = myTinySTL::min(count1, size_ - pos1);
	...
	{
		auto n1 = myTinySTL::min(count1, size_ - pos1);
		auto n2 = myTinySTL::min(count2, other.size_ - pos2);
		...
		swap(basic_string& rhs) noexcept
	{
		if (this != &rhs)
		{
			myTinySTL::swap(buffer_, rhs.buffer_);
	...
	{
		const auto init_size = myTinySTL::max(static_cast<size_type>(STRING_INIT_SIZE), n + 1);
	...
	template <class Iter>
	void basic_string<CharType, CharTraits>::
		copy_init(Iter first, Iter last, myTinySTL::input_iterator_tag)
	{
	...
	template <class Iter>
	void basic_string<CharType, CharTraits>::
		copy_init(Iter first, Iter last, myTinySTL::forward_iterator_tag)
	{
		...
		try
		{
			...
			size_ = n;
			cap_ = init_size;
			myTinySTL::uninitialized_copy(first, last, buffer_);
		}
		catch (...)
		{
			buffer_ = nullptr;
	...
	{
		const auto init_size = myTinySTL::max(static_cast<size_type>(STRING_INIT_SIZE), count + 1);
		buffer_ = data_allocator::allocate(init_size);
		...
		append_range(Iter first, Iter last)
	{
		...
		myTinySTL::uninitialized_copy_n(first, n, buffer_ + size_);
		size_ += n;
		return *this;
	}
	...
	{
		const auto new_cap = myTinySTL::max(cap_ + need, cap_ + (cap_ >> 1));
		...
		reallocate_and_fill(iterator pos, size_type n, value_type ch)
	{
		...
		const auto new_cap = myTinySTL::max(old_cap + n, old_cap + (old_cap >> 1));
		auto new_buffer = data_allocator::allocate(new_cap);
		...
		reallocate_and_copy(iterator pos, const_iterator first, const_iterator last)
	{
		...
		auto new_buffer = data_allocator::allocate(new_cap);
		auto e1 = char_traits::move(new_buffer, buffer_, r) + r;
		auto e2 = myTinySTL::uninitialized_copy_n(first, n, e1) + n;
		char_traits::move(e2, buffer_ + r, size_ - r);
		data_allocator::deallocate(buffer_, old_cap);
		buffer_ = new_buffer;
		size_ += n;
	...
	basic_string<CharType, CharTraits>
		operator+(basic_string<CharType, CharTraits>&& lhs,
			const basic_string<CharType, CharTraits>& rhs)
	{
		basic_string<CharType, CharTraits> tmp(myTinySTL::move(lhs));
		tmp.append(rhs);
		return tmp;
	}
	...
	{
		basic_string<CharType, CharTraits> tmp(myTinySTL::move(rhs));
