...
namespace myTinySTL {
#ifdef max
#pragma meaage("#undefing marco max")
#udef max
#endif //max
...
struct deque_buf_size {
	//constexpr就要求编译器在编译时检测这个表达式本身是不是编译时可以计算出常量值的。
...
struct deque_iterator :public iterator<random_access_iterator_tag, T> {
	...
	//构造、复制、移动函数
	//noexcept关键字修饰的函数不会在执行过程中抛出异常。
	//如果标记为noexcept的函数抛出异常，那么编译器会调用std::terminate()函数中断程序执行。
	...
	deque_iterator(iterator&& rhs)noexcept :cur(rhs.cur), first(rhs.first), last(rhs.last), node(rhs.node) {
		rhs.cur = nullptr; rhs.first = nullptr; rhs.last = nullptr; rhs.node = nullptr;
	}
	...
	//转到另一个缓冲区
	void set_mode(map_pointer new_node) {
		node = new_node;
		first = *new_node;
	...
	ponter operator->()const { return cur; }
...
public:
	...
	typedef myTinySTL::reverse_iterator_t <iterator>        reverse_iterator_t;
	typedef myTinySTL::reverse_iterator_t <const_iterator>  const_reverse_iterator_t;
...
public:
	...
	{
		copy_init(ilist.begin(), ilist.end(), myTinySTL::forward_iterator_tag());
	}

	deque(const deque& rhs) {
		copy_init(rhs.begin_(), rhs.end_, myTinySTL::forward_iterator_tag());
	}

	deque(deque&& rhs) noexcept
		:begin_(myTinySTL::move(rhs.begin_)),
		end_(myTinySTL::move(rhs.end_)),
		map_(rhs.map_),
	...
	reverse_iterator_t rbegin()noexcept {
		return reverse_iterator_t(end());
	}
	const_reverse_iterator_t rbegin() const noexcept {
		return reverse_iterator_t(end());
	}
	reverse_iterator_t rend() noexcept {
		return reverse_iterator_t(begin());
	}
	const_reverse_iterator_t rend() const noexcept {
		return reverse_iterator_t(begin());
	}
	...
	const_reverse_iterator_t crbegin()const noexcept {
		return rbegin();
	}
	const_reverse_iterator_t crend()const noexcept {
		return rend();
	}
	...
	{
		copy_assign(ilist.begin(), ilist.end(), myTinySTL::forward_iterator_tag{});
	}
	...
	void     push_front(value_type&& value) { emplace_front(myTinySTL::move(value)); }
	void     push_back(value_type&& value) { emplace_back(myTinySTL::move(value)); }
...
deque<T>& deque<T>::operator=(const deque& rhs)
{
	if (this != &rhs)
	{
		...
		if (len >= rhs.size())
		{
			erase(myTinySTL::copy(rhs.begin_, rhs.end_, begin_), end_);
		}
		else
		{
			iterator mid = rhs.begin() + static_cast<difference_type>(len);
			myTinySTL::copy(rhs.begin_, mid, begin_);
...
void deque<T>::emplace_front(Args&& ...args)
{
	...
	{
		data_allocator::construct(begin_.cur - 1, myTinySTL::forward<Args>(args)...);
		--begin_.cur;
	}
	else
	{
		...
		{
			--begin_;
			data_allocator::construct(begin_.cur, myTinySTL::forward<Args>(args)...);
		}
		catch (...)
		{
			++begin_;
...
void deque<T>::emplace_back(Args&& ...args)
{
	...
	{
		data_allocator::construct(end_.cur, myTinySTL::forward<Args>(args)...);
		++end_.cur;
	}
	else
	{
...
typename deque<T>::iterator deque<T>::emplace(iterator pos, Args&& ...args)
{
	...
	{
		emplace_front(myTinySTL::forward<Args>(args)...);
		return begin_;
	}
	else if (pos.cur == end_.cur)
	{
		emplace_back(myTinySTL::forward<Args>(args)...);
		return end_;
	}
	return insert_aux(pos, myTinySTL::forward<Args>(args)...);
}
...
deque<T>::insert(iterator position, value_type&& value)
{
	if (position.cur == begin_.cur)
	{
		emplace_front(myTinySTL::move(value));
		return begin_;
	}
	else if (position.cur == end_.cur)
	{
	...
	{
		return insert_aux(position, myTinySTL::move(value));
	}
}
...
void deque<T>::insert(iterator position, size_type n, const value_type& value)
{
	...
	else if (position.cur == end_.cur)
	{
		...
		auto new_end = end_ + n;
		myTinySTL::uninitialized_fill_n(end_, n, value);
		end_ = new_end;
	}
	else
	{
...
deque<T>::erase(iterator position)
{
	...
	{
		myTinySTL::copy_backward(begin_, position, next);
		pop_front();
	}
	else
	{
		myTinySTL::copy(next, end_, position);
		pop_back();
	}
...
deque<T>::erase(iterator first, iterator last)
{
	...
	else
	{
		...
		{
			myTinySTL::copy_backward(begin_, first, last);
		...
		{
			myTinySTL::copy(last, end_, first);
...
void deque<T>::clear()
{
	...
	{ // 有两个以上的缓冲区
		myTinySTL::destroy(begin_.cur, begin_.last);
		myTinySTL::destroy(end_.first, end_.cur);
	}
	else
	{
		myTinySTL::destroy(begin_.cur, end_.cur);
	}
	shrink_to_fit();
	end_ = begin_;
}
...
void deque<T>::swap(deque& rhs) noexcept
{
	if (this != &rhs)
	{
		...
		myTinySTL::swap(map_, rhs.map_);
		myTinySTL::swap(map_size_, rhs.map_size_);
	}
}
...
map_init(size_type nElem)
{
	...
	map_size_ = myTinySTL::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
	try
	{
...
void deque<T>::fill_init(size_type n, const value_type& value)
{
	...
	if (n != 0)
	{
		...
		myTinySTL::uninitialized_fill(end_.first, end_.cur, value);
	}
}
...
copy_init(FIter first, FIter last, forward_iterator_tag)
{
	...
	{
		auto next = first;
		myTinySTL::advance(next, buffer_size);
		myTinySTL::uninitialized_copy(first, next, *cur);
		first = next;
	}
	myTinySTL::uninitialized_copy(first, last, end_.first);
}
...
copy_assign(FIter first, FIter last, forward_iterator_tag)
{
	...
	const size_type len2 = myTinySTL::distance(first, last);
	if (len1 < len2)
	{
		auto next = first;
		myTinySTL::advance(next, len1);
		myTinySTL::copy(first, next, begin_);
		insert_dispatch(end_, next, last, forward_iterator_tag{});
	}
	else
	{
		erase(myTinySTL::copy(first, last, begin_), end_);
	}
}
...
{
	const size_type elems_before = position - begin_;
	value_type value_copy = value_type(myTinySTL::forward<Args>(args)...);
	if (elems_before < (size() / 2))
	{ // 在前半段插入
		emplace_front(front());
		auto front1 = begin_;
		...
		auto pos = position;
		++pos;
		myTinySTL::copy(front2, pos, front1);
	}
	else
	{ // 在后半段插入
		emplace_back(back());
	...
	*position = myTinySTL::move(value_copy);
	return position;
}
...
fill_insert(iterator position, size_type n, const value_type& value)
{
	...
	if (elems_before < (len / 2))
	{
		...
		try
		{
			if (elems_before >= n)
			{
				...
				myTinySTL::uninitialized_copy(begin_, begin_n, new_begin);
				begin_ = new_begin;
				myTinySTL::copy(begin_n, position, old_begin);
				myTinySTL::fill(position - n, position, value_copy);
			}
			else
			{
				myTinySTL::uninitialized_fill(
					myTinySTL::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
				begin_ = new_begin;
				myTinySTL::fill(old_begin, position, value_copy);
			}
		}
		catch (...)
		{
	...
	else
	{
		...
		try
		{
			...
			{
				auto end_n = end_ - n;
				myTinySTL::uninitialized_copy(end_n, end_, end_);
				end_ = new_end;
				myTinySTL::copy_backward(position, end_n, old_end);
				myTinySTL::fill(position, position + n, value_copy);
			}
			else
			{
				myTinySTL::uninitialized_fill(end_, position + n, value_copy);
				myTinySTL::uninitialized_copy(position, end_, position + n);
				end_ = new_end;
				myTinySTL::fill(position, old_end, value_copy);
			}
		}
		catch (...)
		{
...
copy_insert(iterator position, FIter first, FIter last, size_type n)
{
	...
	if (elems_before < (len / 2))
	{
		...
		try
		{
			...
			{
				auto mid = first;
				myTinySTL::advance(mid, n - elems_before);
				myTinySTL::uninitialized_copy(first, mid,
					myTinySTL::uninitialized_copy(begin_, position, new_begin));
				begin_ = new_begin;
				myTinySTL::copy(mid, last, old_begin);
			}
		}
		catch (...)
		{
	...
	else
	{
		...
		try
		{
			...
			{
				auto mid = first;
				myTinySTL::advance(mid, elems_after);
				myTinySTL::uninitialized_copy(position, end_,
					myTinySTL::uninitialized_copy(mid, last, end_));
				end_ = new_end;
...
insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag)
{
	...
	if (position.cur == begin_.cur)
	{
		...
		try
		{
			myTinySTL::uninitialized_copy(first, last, new_begin);
			begin_ = new_begin;
		}
		catch (...)
		{
	...
	else if (position.cur == end_.cur)
	{
		...
		try
		{
			myTinySTL::uninitialized_copy(first, last, end_);
			end_ = new_end;
		}
		catch (...)
		{
...
{
	const size_type new_map_size = myTinySTL::max(map_size_ << 1,
...
bool operator<(const deque<T>& lhs, const deque<T>& rhs)
{
	return myTinySTL::lexicographical_compare(
