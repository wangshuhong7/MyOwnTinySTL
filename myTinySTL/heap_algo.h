...
namespace myTinySTL
{
	...
	{
		myTinySTL::push_heap_aux(first, (last - first) - 1, static_cast<Distance>(0), *(last - 1));
	}

	template <class RandomIter>
	void push_heap(RandomIter first, RandomIter last)
	{ // 新元素应该已置于底部容器的最尾端
		myTinySTL::push_heap_d(first, last, distance_type(first));
	}
	...
	{
		myTinySTL::push_heap_aux(first, (last - first) - 1, static_cast<Distance>(0),
			*(last - 1), comp);
	}
	...
	{
		myTinySTL::push_heap_d(first, last, distance_type(first), comp);
	}
	...
	void adjust_heap(RandomIter first, Distance holeIndex, Distance len, T value)
	{
		...
		myTinySTL::push_heap_aux(first, holeIndex, topIndex, value);
	}
		...
		Distance*)
	{
		...
		*result = *first;
		myTinySTL::adjust_heap(first, static_cast<Distance>(0), last - first, value);
	}

	template <class RandomIter>
	void pop_heap(RandomIter first, RandomIter last)
	{
		myTinySTL::pop_heap_aux(first, last - 1, last - 1, *(last - 1), distance_type(first));
	}
		...
		Compared comp)
	{
		...
		// 再执行一次上溯(percolate up)过程
		myTinySTL::push_heap_aux(first, holeIndex, topIndex, value, comp);
	}
		...
		T value, Distance*, Compared comp)
	{
		*result = *first;  // 先将尾指设置成首值，即尾指为欲求结果
		myTinySTL::adjust_heap(first, static_cast<Distance>(0), last - first, value, comp);
	}

	template <class RandomIter, class Compared>
	void pop_heap(RandomIter first, RandomIter last, Compared comp)
	{
		myTinySTL::pop_heap_aux(first, last - 1, last - 1, *(last - 1),
	...
	void sort_heap(RandomIter first, RandomIter last, Compared comp)
	{
		while (last - first > 1)
		{
			myTinySTL::pop_heap(first, last--, comp);
		}
	}
	...
	void make_heap_aux(RandomIter first, RandomIter last, Distance*)
	{
		...
		{
			// 重排以 holeIndex 为首的子树
			myTinySTL::adjust_heap(first, holeIndex, len, *(first + holeIndex));
			if (holeIndex == 0)
				return;
			holeIndex--;
		}
	...
	{
		myTinySTL::make_heap_aux(first, last, distance_type(first));;
	}
	...
	void make_heap_aux(RandomIter first, RandomIter last, Distance*, Compared comp)
	{
		...
		{
			// 重排以 holeIndex 为首的子树
			myTinySTL::adjust_heap(first, holeIndex, len, *(first + holeIndex), comp);
			if (holeIndex == 0)
				return;
			holeIndex--;
		}
	...
	{
		myTinySTL::make_heap_aux(first, last, distance_type(first), comp);
	}
