...
//
// 异常保证：
// myTinySTL::list<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
namespace myTinySTL
{
	...
	// list 的迭代器设计
	template <class T>
	struct list_iterator : public myTinySTL::iterator<myTinySTL::bidirectional_iterator_tag, T>
	{
	...
	public:
		...
		typedef myTinySTL::allocator<list_node_base<T>>      base_allocator;
		typedef myTinySTL::allocator<list_node<T>>           node_allocator;
	...
	public:
		...
		void     emplace_front(Args&& ...args)
		{
			...
			auto link_node = create_node(myTinySTL::forward<Args>(args)...);
		...
		{
			THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
			auto link_node = create_node(myTinySTL::move(value));
			++size_;
			return link_iter_node(pos, link_node->as_base());
		}
		...
		{
			myTinySTL::swap(node_, rhs.node_);
		...
		void sort()
		{
			list_sort(begin(), end(), size(), myTinySTL::less<T>());
		}
	...
	void list<T>::reverse()
	{
		...
		{
			myTinySTL::swap(i.node_->prev, i.node_->next);
			i.node_ = i.node_->prev;
		}
		myTinySTL::swap(e.node_->prev, e.node_->next);
	}
		...
		list<T>::create_node(Args&& ...args)
	{
		...
		try
		{
			data_allocator::construct(myTinySTL::address_of(p->value), myTinySTL::forward<Args>(args)...);
			p->prev = nullptr;
			p->next = nullptr;
		}
		catch (...)
	...
	{
		data_allocator::destroy(myTinySTL::address_of(p->value));
		node_allocator::deallocate(p);
	}
	...
	bool operator<(const list<T>& lhs, const list<T>& rhs)
	{
		return myTinySTL::lexicographical_compare(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
	}
