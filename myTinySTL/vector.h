...
//
// 异常保证：
// myTinySTL::vecotr<T> 满足基本异常保证，部分函数无异常保证，并对以下函数做强异常安全保证：
//   * emplace
//   * emplace_back
//   * push_back
...
#undef min
	...
	class vector
	{
		static_assert(!std::is_same<bool, T>::value, "vector<bool> is abandoned in myTinySTL");
	public:
		// vector 的嵌套型别定义
	...
	vector<T>& vector<T>::operator=(const vector& rhs)
	{
		if (this != &rhs)
		{
			...
			{
				auto i = myTinySTL::copy(rhs.begin(), rhs.end(), begin());
		...
		vector<T>::emplace(const_iterator pos, Args&& ...args)
	{
		...
		else if (end_ != cap_)
		{
			...
			++new_end;
			myTinySTL::copy_backward(xpos, end_ - 1, end_);
			*xpos = value_type(myTinySTL::forward<Args>(args)...);
		}
		else
		{
			reallocate_emplace(xpos, myTinySTL::forward<Args>(args)...);
		}
		return begin() + n;
	}
		...
		vector<T>::insert(const_iterator pos, const value_type& value)
	{
		...
		else if (end_ != cap_)
		{
			...
			myTinySTL::copy_backward(xpos, end_ - 1, end_);
			*xpos = myTinySTL::move(value_copy);
			end_ = new_end;
		}
		else
		{
		...
		vector<T>::erase(const_iterator first, const_iterator last)
	{
		...
		data_allocator::destroy(myTinySTL::move(r + (last - first), end_, r), end_);
		...
		copy_assign(FIter first, FIter last, forward_iterator_tag)
	{
		...
		{
			auto mid = first;
			myTinySTL::advance(mid, size());
			myTinySTL::copy(first, mid, begin_);
			auto new_end = myTinySTL::uninitialized_copy(mid, last, end_);
			end_ = new_end;
		}
	}
	...
	void vector<T>::reallocate_insert(iterator pos, const value_type& value)
	{
		...
		{
			new_end = myTinySTL::uninitialized_move(begin_, pos, new_begin);
			data_allocator::construct(myTinySTL::address_of(*new_end), value_copy);
			++new_end;
			new_end = myTinySTL::uninitialized_move(pos, end_, new_end);
		}
		catch (...)
		...
		fill_insert(iterator pos, size_type n, const value_type& value)
	{
		...
		{ // 如果备用空间大于等于增加的空间
			...
			{
				end_ = myTinySTL::uninitialized_fill_n(end_, n - after_elems, value_copy);
				end_ = myTinySTL::uninitialized_move(pos, old_end, end_);
				myTinySTL::uninitialized_fill_n(pos, after_elems, value_copy);
			}
		}
		else
		{ // 如果备用空间不足
			...
			{
				new_end = myTinySTL::uninitialized_move(begin_, pos, new_begin);
				new_end = myTinySTL::uninitialized_fill_n(new_end, n, value);
				new_end = myTinySTL::uninitialized_move(pos, end_, new_end);
			}
			catch (...)
			{
		...
		copy_insert(iterator pos, IIter first, IIter last)
	{
		...
		{ // 如果备用空间大小足够
			...
			{
				auto mid = first;
				myTinySTL::advance(mid, after_elems);
	...
	void vector<T>::reinsert(size_type size)
	{
		...
		try
		{
			myTinySTL::uninitialized_move(begin_, end_, new_begin);
		}
		catch (...)
		{
	...
	bool operator<(const vector<T>& lhs, const vector<T>& rhs)
	{
		return myTinySTL::lexicographical_compare(lhs.begin(), lhs.end(), rhs.begin(), lhs.end());
	}
